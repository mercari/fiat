...
public class RedisPermissionsRepository implements PermissionsRepository {
  ...
  public Map<String, UserPermission> getAllById() {
    Table<String, ResourceType, Response<Map<byte[], byte[]>>> responseTable = getAllFromRedis();
  ...
  public Map<String, UserPermission> getAllByRoles(List<String> anyRoles) {
    ...
    Table<String, ResourceType, Response<Map<byte[], byte[]>>> responseTable =
  ...
  private UserPermission getUserPermission(String userId, RawUserPermission raw) {
    ...
    for (Map.Entry<ResourceType, Response<Map<byte[], byte[]>>> entry : raw.entrySet()) {
      ResourceType r = entry.getKey();

      Map<byte[] /*resourceName*/, byte[] /*resource json*/> resourceMap = entry.getValue().get();
  ...
  private Table<String, ResourceType, Response<Map<byte[], byte[]>>> getAllFromRedis() {
    try {
      Set<String> allUsers = scanSet(SafeEncoder.encode(allUsersKey));
  ...
  private Table<String, ResourceType, Response<Map<byte[], byte[]>>> getAllFromRedis(
    ...
    try {
      final Table<String, ResourceType, Response<Map<byte[], byte[]>>> responseTable =
          ArrayTable.create(
              userIds,
              new ArrayIterator<>(
              ...
              for (String userId : userIdSubset) {
                resources.stream()
                    ...
                    .forEach(
                        r -> {
                          responseTable.put(
                              userId, r, p.hgetAll(SafeEncoder.encode(userKey(userId, r))));
                        });
              }
              p.sync();
            });
  ...
  private class RawUserPermission extends HashMap<ResourceType, Response<Map<byte[], byte[]>>> {
    ...
    RawUserPermission(Map<ResourceType, Response<Map<byte[], byte[]>>> source) {
      super(source);
    }
  }
